...
<body>
            ...
            <table class="User-table">
                ...
                <%
                    ...
                    try {
                        ...
                        "jdbc:mysql://database:3306/ITEMCONTROL?useSSL=false&serverTimezone=UTC&allowPublicKeyRetrieval=true", 
                        "appuser", 
                         "apppass");
                ...
                String usuarioDuplicado = request.getParameter("usuario");

               if ("duplicado".equals(error)) {
               %>
               <div style="color:red; font-weight:bold;">
                Error: El usuario "<%= usuarioDuplicado %>" ya existe. Intenta con otro nombre.
                </div>
                <%
                } else if ("bd".equals(error)) {
                %>
                <div style="color:red; font-weight:bold;">
                 Error inesperado en la base de datos. Intenta más tarde.
                 </div>
                 <%
                 }
                 %>
    ...
    // Mostrar formulario de inserción
        ...
        function validarFormulario() {
            ...
            // Validar duplicados solo si se está insertando un nuevo usuario
            if (document.getElementById('formAction').value === 'insert') {
                if (usuariosExistentes.includes(username)) {
                    document.getElementById("clientError").textContent = "El nombre de usuario ya existe. Usa uno diferente.";
                    document.getElementById("clientError").classList.remove("hidden");
                    return false;
                }
            }
            // En modo edición, si el nombre ha cambiado, también validar duplicados
        if (action === 'edit' && username !== oldUsername && usuariosExistentes.includes(username)) {
