    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if("insert".equals(action)) {
                ...
                String sql = "INSERT INTO Productos (lote_Producto, nombre_De_Producto, fecha_De_Caducidad, "
                           + "proveedor, precio_De_Venta, cantidad_En_Stock, departamento, marca_Del_Producto, "
                           + "fecha_De_Ingreso, estado) "
                           + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                ...
                String sql = "UPDATE Productos SET nombre_De_Producto = ?, fecha_De_Caducidad = ?, "
                           + "proveedor = ?, precio_De_Venta = ?, cantidad_En_Stock = ?, departamento = ?, "
                           + "marca_Del_Producto = ?, fecha_De_Ingreso = ?, estado = ? "
                           + "WHERE lote_Producto = ?";
                ...
                pstmt.setDate(2, Date.valueOf(fechaCaducidad));
                pstmt.setString(3, proveedor);
                pstmt.setDouble(4, Double.parseDouble(precioParam));
                pstmt.setInt(5, Integer.parseInt(stockParam));
                pstmt.setString(6, departamento);
                pstmt.setString(7, marca.isEmpty() ? null : marca);
                pstmt.setDate(8, Date.valueOf(fechaIngreso));
                pstmt.setString(9, estado);
                pstmt.setLong(10, Long.parseLong(loteParam));
